...
export function applyColorSettingsToDOM(settings: ColorSettings): void {
	...
	const CHROMA = 0.2
	const userColour = `oklch(${settings.lightness} ${CHROMA} ${settings.hue})`
	const blackOrWhite = settings.theme === 'light' ? 'oklch(0 0 0)' : 'oklch(1 0 0)'

	document.documentElement.style.setProperty('--user-colour', userColour)
	document.documentElement.style.setProperty('--black-or-white', blackOrWhite)
}
